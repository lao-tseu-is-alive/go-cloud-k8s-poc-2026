...
// Route registration for the thing module.
//
// Transport: ConnectRPC handler wrapped by a Vanguard transcoder (Connect + gRPC
// + gRPC-Web on the standard path /goeland.v1.ThingService/...), plus the REST
...
func (m *Module) VanguardServices() []*vanguard.Service {
	_, handler := goelandv1connect.NewThingServiceHandler(m.connect, m.connectOption())
	return []*vanguard.Service{
		vanguard.NewService(goelandv1connect.ThingServiceName, handler),
	}
}

// ServiceNames returns the fully-qualified gRPC service names owned by this module.
func (m *Module) ServiceNames() []string {
	return []string{goelandv1connect.ThingServiceName}
}
...
func (m *Module) RegisterRoutes(mux *http.ServeMux) error {
	...
	if err != nil {
		return fmt.Errorf("thing module: build transcoder: %w", err)
	}
	mux.Handle("/", http.MaxBytesHandler(transcoder, maxRequestBodyBytes))
	m.deps.Logger.Info("thing module routes registered", "service", goelandv1connect.ThingServiceName)
	return nil
}
