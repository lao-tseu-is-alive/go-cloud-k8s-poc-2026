...
// Route registration for the case module.
//
// Transport: ConnectRPC handler wrapped by a Vanguard transcoder (Connect + gRPC
// + gRPC-Web on the standard path /goeland.v1.CaseService/...), plus the REST
...
func (m *Module) VanguardServices() []*vanguard.Service {
	_, handler := goelandv1connect.NewCaseServiceHandler(m.connect, m.connectOption())
	return []*vanguard.Service{
		vanguard.NewService(goelandv1connect.CaseServiceName, handler),
	}
}

// ServiceNames returns the fully-qualified gRPC service names owned by this module.
func (m *Module) ServiceNames() []string {
	return []string{goelandv1connect.CaseServiceName}
}
...
func (m *Module) RegisterRoutes(mux *http.ServeMux) error {
	...
	if err != nil {
		return fmt.Errorf("case module: build transcoder: %w", err)
	}
	mux.Handle("/", http.MaxBytesHandler(transcoder, maxRequestBodyBytes))
	m.deps.Logger.Info("case module routes registered", "service", goelandv1connect.CaseServiceName)
	return nil
}
