...
// Route registration for the actor module.
//
// Transport: ConnectRPC handler wrapped by a Vanguard transcoder (Connect + gRPC
// + gRPC-Web on the standard path /goeland.v1.ActorService/...), plus the REST
// bindings declared via google.api.http.
//
...
func (m *Module) VanguardServices() []*vanguard.Service {
	_, handler := goelandv1connect.NewActorServiceHandler(m.connect, m.connectOption())
	return []*vanguard.Service{
		vanguard.NewService(goelandv1connect.ActorServiceName, handler),
	}
}

// ServiceNames returns the fully-qualified gRPC service names owned by this module.
func (m *Module) ServiceNames() []string {
	return []string{goelandv1connect.ActorServiceName}
}
...
func (m *Module) RegisterRoutes(mux *http.ServeMux) error {
	...
	if err != nil {
		return fmt.Errorf("actor module: build transcoder: %w", err)
	}
	mux.Handle("/", http.MaxBytesHandler(transcoder, maxRequestBodyBytes))
	m.deps.Logger.Info("actor module routes registered", "service", goelandv1connect.ActorServiceName)
	return nil
}
