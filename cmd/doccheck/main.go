...
func run(ctx context.Context, args []string, output io.Writer) error {
	...
	if *scope == "all" || *scope == "atlas" {
		options := atlasOptions{path: *atlasPath, versionFile: *versionFile, versionName: *versionName, bannerPrefix: *bannerPrefix}
		atlasFindings, err := checkAtlas(os.DirFS(*root), files, options)
		if err != nil {
			return err
		}
...
func checkAtlas(source fs.FS, files []string, options atlasOptions) ([]finding, error) {
	...
	atlasVersionPattern := options.bannerPattern()
